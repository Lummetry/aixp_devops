...
if [ "$1" == "-v" ] || [ "$1" == "-vv" ] || [ "$1" == "-vvv" ]; then
    ANSIBLE_FORCE_COLOR=true ansible-playbook -vvv deploy-config.yml | tee "./logs/ansible.log"
    exit 0
else
    ANSIBLE_FORCE_COLOR=true ansible-playbook deploy-config.yml | tee "./logs/ansible.log"
    exit 0
fi
