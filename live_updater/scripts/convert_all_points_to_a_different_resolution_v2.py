...
with open("nodes_pipelines_resolution.json", "r") as f:
...
if __name__ == "__main__":
  ...
  for node, dct_pipeline_names in TARGET_NODES.items():
    if not session.wait_for_node(node):
      session.P(f"Node {node} did not send any heartbeat.", color='r')
      continue

    for pipeline_name, dct_resolution in dct_pipeline_names.items():
      pipeline: Pipeline = session.attach_to_pipeline(node=node, name=pipeline_name)
      configured_h: int = pipeline.config.get("DEFAULT_H", pipeline.config.get("FRAME_H", dct_resolution["IMAGE_H"]))
      configured_w: int = pipeline.config.get("DEFAULT_W", pipeline.config.get("FRAME_W", dct_resolution["IMAGE_W"]))

      if configured_h is None or configured_w is None:
        session.P(f"Pipeline {pipeline_name} on node {node} has no resolution configured.", color='r')
        continue
